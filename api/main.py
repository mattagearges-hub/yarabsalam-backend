...
async def chat_endpoint(request: ChatRequest):
    ...
    # تحديد نوع الحظر بناءً على الكلمات المفتاحية المنظفة
    is_medical = any(k in cleaned_message for k in MEDICAL_KEYWORDS)
    is_crisis = any(k in cleaned_message for k in CRISIS_KEYWORDS)

    # 1. إذا كان طلب طبي بحت (أدوية وروشتات) -> نرجع رد مباشر ولا نفتح جرس الإنذار الصوتي
    ...
    needs_search = (
        ...
        and not is_crisis  # الحالات الحادة لا تحتاج بحثاً روحيّاً بل احتواء فوري
    ...
    # ── [توجيهات الـ System Prompt بعد ضبط اللغة ومنع الأخطاء العامية] ──
...
## ضبط اللغة والأسلوب العامي (صارم جداً):
- تحدث بالعامية المصرية القاهرية السلسة والمفهومة طبيعياً. 
- ممنوع تماماً دمج أداة النفي "مش" مع حرف "الشين" في آخر الكلمة (فلا تقل أبداً: "مش هتنساكش" أو "مش هتاخديكش"، بل قل بشكل صحيح ومفهوم: "مش هتنساك"، "مش هتاخد من وقتك"، "ربنا مش هينساك").
...
3. في الإجابات الطويلة واللاهوتية والأزمات النفسية: ادخلي في الإجابة واحتواء المستخدم مباشرة، وممنوع حشر جملة التعارف وسؤال الاسم إذا كان السياق يقتضي الإنقاذ والرد الفوري."""
    ...
    try:
        ...
        # نرجع الرد النهائي مع علامة تشغيل الإنذار بناءً على حالة الخطر
        return {
            "answer": answer or "عذراً، لم أستطع توليد رد مناسب حالياً. جرب تسألني تاني.",
            "trigger_alarm": is_crisis  # سيرسل True فقط في حالات الخطر النفسي الحاد لتشغيل صوت الـ SOS في الفرونت
        }
